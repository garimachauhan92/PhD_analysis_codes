...
parser.add_argument('-vH',
					action='store',dest='vvir_cut_halo',type=str,default=None,
					help='zcut for the halo')

parser.add_argument('-vC',
					action='store',dest='vvir_cut_central',type=str,default=None,
...
vvir_cut_halo = float(args.vvir_cut_halo)
vvir_cut_central = float(args.vvir_cut_central)
...
if __name__ == '__main__':
	...
	with Pool(num_threads) as pool:
		...
		if group_cat == 'False':
			...
			if len(bunch) == 1:
				total_tasks = len(matched_id_galaxy_sky[bunch[0]:35 + bunch[0]])
				tasks = matched_id_galaxy_sky[bunch[0]: bunch[0]+35]
		...
		for t in tasks:
			...
			temp = [pool.apply(Common_module.function_stacking_vvir_multiprocessing,(t,group_cat,group_func,lightcone_alfalfa,group_func_lightcone, vvir_cut_halo, rvir_cut_halo, vvir_cut_central, rvir_cut_central, rgas_value, 'luminosity', 'True'))]
