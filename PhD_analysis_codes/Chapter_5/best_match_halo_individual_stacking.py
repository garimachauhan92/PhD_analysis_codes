...
def prepare_data(path,subvolumes,filename):
    ...
    fields_read     = {'galaxies':('id_halo_sam', 'id_galaxy_sam','id_galaxy_sky', 'type', 'mvir_hosthalo','mvir_subhalo', 'matom_bulge', 'matom_disk', 'ra', 'dec', 'vpec_r','vpec_x','vpec_y','vpec_z', 'zcos', 'zobs', 'mstars_bulge','mstars_disk', 'id_group_sky', 'mag')} #'vvir_hosthalo', 'vvir_subhalo',
...
for i in range(subvolumes):
    ...
    (id_halo_sam, id_galaxy_sam, id_galaxy_sky, type_g, mvir_hosthalo, mvir_subhalo, matom_bulge, matom_disk, ra, dec, vpec_r,vpec_x,vpec_y,vpec_z, zcos, zobs,mstars_bulge,mstars_disk, id_group_sky, mag) = prepare_data(path_GAMA, i, 'mock_medi_alfalfa') #'mock_medi_alfalfa') #'mocksky')# #, vvir_hosthalo, vvir_subhalo
...
for i in best_match_dictionary.keys():
    ...
    Purtiy_fraction[i] = {'halo_ids': add_halo, 'purity_fraction':add_purity}
...
for i,j in zip(Purtiy_fraction.keys(),range(len(Purtiy_fraction.keys()))):
    ...
    best_match_halo[i] = {'halo_id':yo_halo_id[j], 'purity_fraction':yo[j], 'central_halo_id':np.array(grand_gals_lightcone[(grand_gals_lightcone['GroupID'] == i) & (grand_gals_lightcone['RankIterCen'] == 1)])}
...
matching_ID      = np.unique(grand_group_nfof['GroupID']) 
...
for i in Purtiy_fraction.keys():
    ...
    if len(grand_gals_lightcone[(grand_gals_lightcone['GroupID'] == i) & (grand_gals_lightcone['RankIterCen'] == 1)]) != 0:
        ...
        HI_central_add = int(grand_gals_lightcone[(grand_gals_lightcone['GroupID'] == i) & (grand_gals_lightcone['RankIterCen'] == 1)]['matom_all'])

        HI_satellite_add = int(np.sum(grand_gals_lightcone[(grand_gals_lightcone['GroupID'] == i) & (grand_gals_lightcone['RankIterCen'] != 1)]['matom_all']))

    
        HI_satellite_direct     = np.append(HI_satellite_direct, HI_satellite_add/1.35/h)
        HI_central_direct       = np.append(HI_central_direct, HI_central_add/1.35/h)

        Mvir_group_direct       = np.append(Mvir_group_direct, int(grand_group_lightcone[(grand_group_lightcone['GroupID'] == i) & (grand_group_lightcone['RankIterCen'] == 1)]['MassAfunc']))

   

    Avg_grp_HI = HI_central_add + HI_satellite_add#/len(trial)
    HI_group_direct         = np.append(HI_group_direct, Avg_grp_HI)


HI_central_direct = HI_central_direct[Mvir_group_direct != 0]
HI_group_direct   = HI_group_direct[Mvir_group_direct != 0]
HI_satellite_direct = HI_satellite_direct[Mvir_group_direct != 0]
Mvir_group_direct = Mvir_group_direct[Mvir_group_direct != 0]
...
# plotting_properties_halo(Mvir_spherical_group_5, MHI_spherical_group_5, legend_handles=legendHandles, legend_name='SHARK-ref $(N_{g} \\geq 5)$', colour_line='maroon', alpha=0.5, mean=True)


# Halo_mass_plotting, HI_mass_plotting, HI_mass_low, HI_mass_high = Common_module.halo_value_list(Mvir_group_direct/h, HI_group_direct, mean=True)
# Error_SAM_2         = plt.errorbar(Halo_mass_plotting[4:len(Halo_mass_plotting)-10], (HI_mass_plotting[4:len(Halo_mass_plotting)-10]), yerr=None,marker = "X", mfc = 'k', mec = 'k', c = 'k', capsize=2,ls = '--', markersize=15, linewidth=1, barsabove=True, capthick=2, label='HI stacking $(N_{g} \\geq 5)$')


# Halo_intrinsic_mean, HI_intrinsic_mean, a,b = Common_module.halo_value_list(Mvir_intrinsic, HI_intrinsic, mean=True)
# Halo_GAMA_mean, HI_GAMA_mean, a,b = Common_module.halo_value_list(Mvir_group_direct/h, HI_group_direct, mean=True)
...
# plt.savefig('NG_DINGO_trial_new.png')
# plt.close()
